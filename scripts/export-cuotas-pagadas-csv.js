...
class HttpRequestError extends Error {
  constructor(statusCode, url, body) {
    super(`HTTP ${statusCode} al consultar ${url}\n${body}`);
...
async function main() {
  ...
  const limit = toPositiveInt(options.limit, 1000, 1000);
  let offset = 0;
  let totalRows = 0;
  let page = 0;
  ...
  try {
    while (true) {
      page += 1;
      const url = buildUrl(options, limit, offset);
      ...
      console.log(
        `[export-cuotas-pagadas] pagina=${page} offset=${offset} rows=${rows.length} total=${totalRows} tieneMas=${Boolean(payload.tieneMas)}`,
      );

      if (!payload.tieneMas || rows.length === 0) {
        break;
      }
  ...
  console.log(
    `[export-cuotas-pagadas] exportacion finalizada. filas=${totalRows} archivo=${outputPath} tiempo=${seconds}s`,
  );
}
...
function parseArgs(args) {
  ...
  const values = {
    baseUrl: `http://127.0.0.1:${port}/cuotas-pagadas`,
    limit: '1000',
    retries: '5',
    timeoutMs: '300000',
    retryDelayMs: '2000',
...
async function fetchJsonWithRetry(url, options) {
  ...
  for (let attempt = 1; attempt <= retries + 1; attempt += 1) {
    try {
      ...
      console.warn(
        `[export-cuotas-pagadas] intento=${attempt} fallo=${error.message} reintentandoEnMs=${delayMs}`,
      );
      await sleep(delayMs);
    }
  }
...
function shouldRetry(error) {
  ...
  if (![408, 429, 502, 503, 504].includes(error.statusCode)) {
    return false;
  }

  const normalizedBody = String(error.body || '').toLowerCase();
  if (
    normalizedBody.includes('cbi_cuotas_pagadas_v') ||
    normalizedBody.includes('read-only') ||
    normalizedBody.includes('ora-04045') ||
    normalizedBody.includes('ora-16000')
  ) {
    return false;
  }
...
function buildUrl(options, limit, offset) {
  const url = new URL(options.baseUrl);
  url.searchParams.set('limite', String(limit));
  url.searchParams.set('offset', String(offset));
  url.searchParams.set('incluirTotal', 'false');

  setParam(url, 'numeroContrato', options.numeroContrato);
  setParam(url, 'documento', options.documento);
  setParam(url, 'idCliente', options.idCliente);
  setParam(url, 'sucursal', options.sucursal);
  setParam(url, 'estadoActualContrato', options.estadoActualContrato);
  setParam(url, 'moneda', options.moneda);
...
function defaultOutputPath() {
  ...
  return path.join('exports', `cuotas-pagadas-${timestamp}.csv`);
}
...
main().catch((error) => {
  console.error(`[export-cuotas-pagadas] ${error.message}`);
  process.exit(1);
});
